...
func resourceClusterCreate(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		if e, ok := err.(*project.CreateClusterDefault); ok && e.Payload != nil && e.Payload.Error != nil && e.Payload.Error.Message != nil {
			return fmt.Errorf("unable to create cluster for project '%s': %s: %s", pID, *e.Payload.Error.Message, err)
		}
	...
	return resource.Retry(d.Timeout(schema.TimeoutDelete), func() *resource.RetryError {
		if !deleteSent {
			...
			if err != nil {
				...
				return resource.NonRetryableError(fmt.Errorf("unable to delete cluster '%s': %v", d.Id(), err))
			}
			deleteSent = true
		}
		p := project.NewGetClusterParams()
